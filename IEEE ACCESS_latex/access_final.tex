...
\item We introduce a novel loss function that combines \textbf{wise intersection-over-union (WIoU)} with \textbf{box morphology consistency loss}. WIoU's center-distance-based sample reweighting alleviates the gradient vanishing problem for small and non-overlapping crack targets by assigning higher loss weights to predictions with larger center deviations. The morphology consistency loss explicitly regularizes predicted boxes toward the elongated aspect ratios characteristic of pavement cracks in log-space, providing an explicit geometric prior that contrasts with CIoU's relative aspect ratio penalty.
...
Wise-IoU (WIoU)~\cite{tong2024wiou} introduces a center-distance-based sample reweighting mechanism. It assigns higher loss weights to harder samples with larger center deviations, thereby mitigating gradient vanishing for non-overlapping boxes. Recent extensions such as WIoU v3~\cite{zhou2024wiouv3} further optimize the focusing strategy with dynamic non-monotonic terms for elongated crack bounding box regression. We adopt the base WIoU formulation and superimpose an explicit morphology consistency constraint that operates in log-space. This provides both aspect ratio and scale regularization tailored to elongated crack targets.
...
\end{equation}
where $\text{Router}(X)$ denotes the routing network comprising global average pooling, a two-layer multilayer perceptron (MLP) with SiLU activation, and a final linear layer producing logit scores for each expert. The symbol $\Delta^4$ denotes the 4-simplex (the set of probability vectors over the four experts). To enforce sparsity and limit computation, we retain only the top-$k$ experts (we set $k=2$) by masking the remaining weights to zero and re-normalizing the selected weights. The MoE output is:
\begin{equation}
Y = \sum_{i=0}^{3} w_i \cdot \text{Expert}_i(X),
\end{equation}
followed by a $1\times1$ projection and a residual connection. This sparse routing mechanism operates at the feature-map level via global average pooling. It allows the network to adaptively emphasize the most relevant morphological expert branches for each input region. Consequently, both accuracy and computational efficiency are improved compared to uniform convolutions.
...
\midrule
\multicolumn{5}{c}{After Oversampling (Train: 6,599 images, 36,944 instances)} \\
\midrule
...
The long-tail distribution is evident: the five crack categories collectively dominate (77.4\% of all instances), with Transverse Crack alone contributing 48.4\%; Pothole and Repair account for only 2.0\% and 1.8\% of the validation set, respectively, representing the extreme long-tail regime.
...
BMS-YOLO-n achieves improvements on five of six categories. The most substantial gains appear in Repair (+5.0 points) and Transverse Crack (+4.6 points). The improvement on Repair is attributed to the FDDE module's enhanced receptive field, which better captures the irregular boundaries of repaired regions. For Transverse Crack, a thin, horizontal linear pattern, the Morphology Loss provides a direct supervisory signal. This guides the network toward elongated box predictions, directly addressing the aspect-ratio mismatch between the anchor-free prediction head's default isotropic scale priors and crack geometry.
...
The slight decline on Pothole (--0.9 points) is expected given the extreme class imbalance. The test set contains only 17 Pothole instances (1.4\% of all test annotations), and the Repair category has just 29 instances (2.4\%). This places both categories in the long-tail regime where stochastic variation dominates. The overall accuracy gain of 2.4 points confirms that the morphological design benefits linear distress types without substantially harming compact defect detection.
...
As shown in Fig.~\ref{fig:qual1}, a faint transverse crack crosses double-yellow lane markings. YOLOv8n assigns a low confidence score (Transverse Crack: 0.39) and produces a bounding box with excessive vertical padding. BMS-YOLO-n raises the confidence to 0.47 and tightens the box to better follow the crack geometry. This demonstrates the benefit of Morphology Loss in resolving aspect-ratio ambiguity. In Fig.~\ref{fig:qual2}, a dense multi-class scene contains Repair, Oblique Crack, and Longitudinal Crack regions. YOLOv8n fragments the same crack into three to four separate boxes with confidences ranging from 0.61 to 0.80. BMS-YOLO-n, by contrast, produces a more coherent set of detections with higher confidences (Repair: 0.89 vs.\ 0.78; Oblique Crack: 0.83 vs.\ 0.80). This confirms the role of topology-guided fusion in merging spatially adjacent detections along linear crack trajectories. Finally, Fig.~\ref{fig:qual3} shows a co-occurring pothole and transverse crack scenario. Despite the extreme class imbalance that places Pothole in the long-tail regime, BMS-YOLO-n increases Pothole detection confidence from 0.75--0.76 to 0.88--0.89. This indicates that the WIoU loss effectively elevates the quality of rare-class predictions.
